...
<div x-data="{ 
            ...
            <table class="w-full">
                <thead>
                    <tr class="border-b border-border">
                        <th class="px-6 py-3 text-left text-xs font-semibold text-content-tertiary uppercase tracking-wider">Kecamatan</th>
                        <th class="px-6 py-3 text-left text-xs font-semibold text-content-tertiary uppercase tracking-wider">Desa</th>
                        <th class="px-6 py-3 text-left text-xs font-semibold text-content-tertiary uppercase tracking-wider">Dusun</th>
                ...
                <tbody class="divide-y divide-border">
                    ...
                    <tr class="hover:bg-base-page transition-colors">
                        ...
                        <td class="px-6 py-4">
                            <div class="flex items-center gap-2">
                                ...
                                <form method="POST" action="{{ route('master.dusun.destroy', $item) }}" onsubmit="return confirm('Hapus wilayah ini?')">
                                    @csrf @method('DELETE')
                                    <button type="submit" 
                    ...
                    @empty
                    <tr>
                        <td colspan="4" class="px-6 py-10 text-center text-content-tertiary">Belum ada data wilayah.</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
    ...
    <div x-show="showModal" class="fixed inset-0 z-50 flex items-center justify-center" style="display: none;">
        {{-- Backdrop --}}
        <div x-show="showModal" x-transition.opacity class="absolute inset-0 bg-content-primary/40 backdrop-blur-sm" @click="showModal = false"></div>
        ...
        <div x-show="showModal" 
             ...
             class="relative bg-white rounded-xl shadow-modal w-full max-w-md mx-4 z-10 overflow-hidden">
            
            <div class="px-6 py-4 border-b border-border flex justify-between items-center bg-base-page">
                <h3 class="text-lg font-semibold text-content-primary" x-text="mode === 'create' ? 'Tambah Dusun Baru' : 'Edit Data Dusun'"></h3>
            ...
            <form :action="formAction" method="POST">
                ...
                <div class="px-6 py-4 border-t border-border flex justify-end gap-3 bg-base-page">
                    ...
                    <button type="submit" class="btn-primary" x-text="mode === 'create' ? 'Simpan Dusun' : 'Update Dusun'"></button>
                </div>
            </form>
        </div>
    </div>
