...
@section('page-title', 'Tambah Teknisi Baru')

@section('content')

<div class="max-w-2xl">
    <div class="card p-8">
        <form action="{{ route('master.teknisi.store') }}" method="POST">
            @csrf
            <div class="space-y-4">
                <div>
                    <label class="block text-content-secondary text-sm mb-2">Nama Teknisi</label>
                    <input type="text" name="nama_teknisi" value="{{ old('nama_teknisi') }}" class="input-field" required>
                </div>
                <div>
