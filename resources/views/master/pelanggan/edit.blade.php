...
<div class="max-w-4xl">
    ...
    <div class="card p-8">
        <form action="{{ route('master.pelanggan.update', $pelanggan) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        ...
                        @error('nama_pelanggan') <p class="text-status-danger text-xs mt-1">{{ $message }}</p> @enderror
                    </div>

                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            <label class="block text-content-secondary text-sm mb-2">Kecamatan</label>
                            <input type="text" name="kecamatan" value="{{ old('kecamatan', $pelanggan->kecamatan) }}" class="input-field">
                        </div>
                        <div>
                            <label class="block text-content-secondary text-sm mb-2">Desa</label>
                            <input type="text" name="desa" value="{{ old('desa', $pelanggan->desa) }}" class="input-field">
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <select name="dusun_id" class="input-field">
                            ...
                            @foreach($dusun as $d)
                                <option value="{{ $d->id }}" {{ old('dusun_id', $pelanggan->dusun_id) == $d->id ? 'selected' : '' }}>
                                    {{ $d->dusun }} ({{ $d->desa }})
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        ...
                        <select name="bulanan_id" class="input-field">
                            ...
                            @foreach($bulanan as $b)
                                <option value="{{ $b->id }}" {{ old('bulanan_id', $pelanggan->bulanan_id) == $b->id ? 'selected' : '' }}>
                    ...
                    <div>
                        ...
                        <select name="kolektor_id" class="input-field">
                            ...
                            @foreach($kolektor as $k)
                                <option value="{{ $k->id }}" {{ old('kolektor_id', $pelanggan->kolektor_id) == $k->id ? 'selected' : '' }}>
                                    {{ $k->nama_kolektor }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="date" name="tanggal_pemasangan" value="{{ old('tanggal_pemasangan', $pelanggan->tanggal_pemasangan ? date('Y-m-d', strtotime($pelanggan->tanggal_pemasangan)) : '') }}" class="input-field">
                        </div>
                        <div>
                            ...
                            <select name="status_alat" class="input-field" required>
                                <option value="pinjam" {{ old('status_alat', $pelanggan->status_alat) == 'pinjam' ? 'selected' : '' }}>Pinjam</option>
                                <option value="beli" {{ old('status_alat', $pelanggan->status_alat) == 'beli' ? 'selected' : '' }}>Beli</option>
                            </select>
                        </div>
                    </div>
                </div>
            ...
            <div class="mt-8 pt-6 border-t border-border flex justify-end gap-3">
                ...
                <button type="submit" class="btn-primary px-8">Update Data Pelanggan</button>
            </div>
        </form>
    </div>
</div>
